...
このリポジトリは、日本版の仮想政策シミュレーターの初期MVPです。

現時点では、主要導線をモックデータで動かせます。ダッシュボード、政策ターゲット、声の分析、政策案、実行結果の画面を確認できます。AI接続が有効な場合、政策ターゲット決定後に「声の作成」「クラスター分析」「初期政策案・コスト内訳作成」の3段階でJSON Schemaに沿って生成します。
