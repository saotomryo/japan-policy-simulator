...
日本を想定した国レベルの政策について、対象政策の指定、国民・ステークホルダーの想定反応生成、クラスター分析、AIチャットによる影響確認、政策実行結果表示までを1回の流れで体験できるWebアプリケーションにする。
...
3. AIが政策ターゲットを構造化する
4. ユーザーが `声の分析へ進む` を押す
5. AIまたは固定サンプルが関連指標、国民ペルソナの声、初期政策案を生成する
6. アプリが声を賛否、利害、無関心、実務、専門家、国際関係などの観点で表示する
7. クラスターマップと階層クラスタリングで意見構造を見る
8. ユーザーがチャットで、どのクラスターに効果・不利益・反発が出るか確認する
9. ユーザーが必要に応じて政策案をチャットで調整する
10. ユーザーが政策を1回だけ実行する
11. アプリが短期結果、財源上の懸念、属性別反応、影響の大きい分野の詳細レポートを表示する
...
- `声の分析へ進む` はダッシュボードの `政策ターゲットへ進む` と同じ画面上部右側に配置する
- 政策ターゲットを指定するまでは、声の分析と政策案は未生成表示にする
- `data/scenarios/national/dashboard.json` に事前モックがある政策は、固定サンプル生成ではなく用意済みの声・分析・政策案を優先利用する
...
- 国版MVPでは `コスト / 実施内容 / 効果 / 対象属性` のタブで縦長表示を抑える
- コストは総額だけでなく、何に対してどう費用が発生するかをドリルダウンできる形で表示する
- `政策を実行して結果を見る` はダッシュボードと同じ画面上部右側に配置する
...
- 属性別反応の可視化
- 財源・社会保障・実務負荷などの副作用
- 長期影響欄はMVPでは非表示にする。後で復活しやすいよう内部データや表示ブロックは保持してよい
